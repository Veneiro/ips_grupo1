...
public class PrescripcionesModelo {
	...
	public void addPrescripcion(PrescripcionRecord p) {
		...
		db.executeUpdate(sql, p.getNombre(), p.getPaciente_id(),
				p.getIndicaciones(), p.isMedicamento(), p.getCantidad(),
				p.getIntervalo(), p.getDuracion(), p.getFecha(), p.getHora());
	}

	public List<PrescripcionRecord> getListaPrescripcionesPaciente(
			int idPaciente) {
