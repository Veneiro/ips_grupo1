...
public class CitasModificadasMedicoControlador {
	...
	acvm.getBtnConfirmar().addActionListener(e -> SwingUtil.exceptionWrapper(() -> insertToDB()));
	acvm.getBtnBack().addActionListener(e -> SwingUtil.exceptionWrapper(() -> closeWindow()));
	acvm.getBtnHelp().addActionListener(e -> SwingUtil.exceptionWrapper(() -> showHelp()));
	...
	JOptionPane.showMessageDialog(acvm,
		"Seleccione una de " + "las citas revisadas y modificadas por el administrador "
			...
			+ "- Si solo desea salir de la ventana sin aceptar o " + "rechazar clicke en el botón Atrás\n"
	...
	for (int i = 0; i < acvm.getTable().getRowCount(); i++) {
		...
		cdto.setId_paciente(getIdPaciente((String) acvm.getTable().getValueAt(i, 1)));
		cdto.setId_medico(getIdMedico((String) acvm.getTable().getValueAt(i, 2)));
		cdto.setFecha((String) acvm.getTable().getValueAt(i, 3));
		cdto.setHorario_inicio((String) acvm.getTable().getValueAt(i, 4));
		...
		RegistroModelo
			.addRegistro(
				new RegistroDto("Médico " + getMedicoByID(cdto.getId_medico()).getNombre(),
					"Ha modificado la cita propuesta. Médico: "
						+ new MedicoModelo().getListaMedicosById(cdto.getId_medico()).get(0)
							.getNombre()
						+ "Día: " + cdto.getFecha() + "Hora: " + cdto.getHora_entrada() + "-"
						+ cdto.getHora_salida()));
		...
		JOptionPane.showConfirmDialog(acvm, "Are you sure you want to close? The appointments not "
	...
	for (CitaPendienteDto c : citasPendientes) {
	    if (c.getESTADO().equals("MedicoReview") && c.getID_MEDICO() == idMedicoLogin) {
